...
export async function GET(request: NextRequest) {
  try {
    ...
    const totalRevenue =
      periodOrders?.reduce((sum, order) => sum + (order.total_amount || 0), 0) || 0
    const pendingOrders = periodOrders?.filter((order) => order.status === 'pending').length || 0
    const confirmedOrders =
      periodOrders?.filter((order) => order.status === 'confirmed').length || 0
    const processingOrders =
      periodOrders?.filter((order) => order.status === 'processing').length || 0
    const shippedOrders = periodOrders?.filter((order) => order.status === 'shipped').length || 0
    const deliveredOrders =
      periodOrders?.filter((order) => order.status === 'delivered').length || 0
    const cancelledOrders =
      periodOrders?.filter((order) => order.status === 'cancelled').length || 0
